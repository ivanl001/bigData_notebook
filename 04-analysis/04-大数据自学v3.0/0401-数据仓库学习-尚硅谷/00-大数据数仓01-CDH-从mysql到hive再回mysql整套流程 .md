...
* 这个是建表文件**createTable.sql**，方便后期使用
...
* 这个是函数脚本文件**function.sql**，方便随机生成需要的数据
...
### 1.5,执行如上四步步骤即完成mysql准备工作

* 创建数据库后，分别执行1.1, 1.2, 1.3, 1.4的文件后即完成准备工作



## 2, 数据生成

在mysql中执行如下语句即可生成需要的数据

```mysql
CALL init_data('2019-08-04',1000, 200, 300, TRUE);

# 生成日期2019-08-04日数据、订单1000个、用户200个、商品sku300，TRUE代表删除原始数据
```
...
* 搭建工作参考“03-大数据自学v2.0/02-cm和cdh安装”中的内容
* sqoop安装sqoop1即可，一般不使用sqoop2
...
* 需要添加mysql-connector-java.jar到sqoop安装目录和oozie的hdfs对应目录
* 教程中说还需要安装到hadoop的lib目录下，这个应该是不必要的

```shell
mv mysql-connector-java.jar /opt/cloudera/parcels/CDH/lib/sqoop/lib
# 注意要分发到其他的机器上
imrsync.sh /opt/cloudera/parcels/CDH/lib/sqoop/lib/mysql-connector-java.jar

# 然后上传到oozie的对应的sqoop的hdfs目录中
上传到 /user/oozie/share/lib/lib_20190801001140/sqoop
```

### 4.2, 配置yarn的容器内存

* 在执行的之后总是一直的在heart beat，是因为资源分配不够，导致线程锁定，只能一直进行心跳检测
...
* 避免此问题，做如下修改：
* yarn配置中：
* yarn.nodemanager.resource.memory-mb：内存设置为4G或者8G，我设置为8G
* yarn.nodemanager.resource.cpu-vcores：cpu核数，根据具体而定，我设定5个

### 4.3, hue中yarn用户创建

* 在hue中创建新用户yarn
* 并使用yarn进行sqoop的shell脚本的运行，不然的话会报错

```java
org.apache.hadoop.security.AccessControlException: Permission denied: user=root, access=EXECUTE, inode="/user/yarn/.staging":yarn:yarn:drwx------
```
...
```shell
./01-sqoop_import.sh all 2019-08-03
```
...
### 5.3, 在hue的workflow中调用shell文件即可

* 参数具体如图

![image-20190807121914886](assets/image-20190807121914886.png)
...
# 2, 根据参数确定加载数据的日期，如果日期存在，加载给定日期，日期不存在，加载执行当日的昨天的数据
if [ -n "$1" ];
then theDate=$1
...
# 3, 执行的sql语句
sql=" 
load data inpath '/mysql/$APP/db/order_info/$theDate' OVERWRITE into table "$APP".ods_order_info partition(dt='$theDate');

load data inpath '/mysql/$APP/db/order_detail/$theDate' OVERWRITE into table "$APP".ods_order_detail partition(dt='$theDate');

load data inpath '/mysql/$APP/db/sku_info/$theDate' OVERWRITE into table "$APP".ods_sku_info partition(dt='$theDate');

load data inpath '/mysql/$APP/db/user_info/$theDate' OVERWRITE into table "$APP".ods_user_info partition(dt='$theDate');

load data inpath '/mysql/$APP/db/payment_info/$theDate' OVERWRITE into table "$APP".ods_payment_info partition(dt='$theDate');

load data inpath '/mysql/$APP/db/base_category1/$theDate' OVERWRITE into table "$APP".ods_base_category1 partition(dt='$theDate');

load data inpath '/mysql/$APP/db/base_category2/$theDate' OVERWRITE into table "$APP".ods_base_category2 partition(dt='$theDate');

load data inpath '/mysql/$APP/db/base_category3/$theDate' OVERWRITE into table "$APP".ods_base_category3 partition(dt='$theDate'); 
"

# 4, 打印日志信息
echo "--------准备加载数据，日期是：$theDate--------"
echo "执行的语句是：" $sql
echo "执行动作开始：加载数据"
...
### 使用方式：在hue的workflow中调用上面的shell文件即可
...
```shell
# 脚本的使用
./03-sqoop_export.sh  ads_uv_count
```
...
export_data() {
sqoop export \
--connect "jdbc:mysql://centos01:3306/${db_name}?useUnicode=true&characterEncoding=utf-8"  \
--username root \
--password ,. \
--table $1 \
--num-mappers 1 \
